...
(function (global, factory) {
	...
	EventHandler.on(window, EVENT_LOAD_DATA_API$2, () => {
		...
		for (let i = 0, len = carousels.length; i < len; i++) {
			Carousel.carouselInterface(
				carousels[i],
				Data.get(carousels[i], DATA_KEY$9)
			);
		}
	});
	/**
	...
	class Collapse extends BaseComponent {
		...
		show() {
			if (
				...
				this._element.classList.contains(CLASS_NAME_SHOW$7)
			) {
				return;
			}
			...
			if (actives) {
				...
				activesData = tempActiveData
					? Data.get(tempActiveData, DATA_KEY$8)
					: null;
	...
	class Dropdown extends BaseComponent {
		...
		toggle() {
			...
			const isActive = this._element.classList.contains(
				CLASS_NAME_SHOW$6
			);
		...
		show() {
			if (
				this._element.disabled ||
				this._element.classList.contains(CLASS_NAME_DISABLED) ||
				this._menu.classList.contains(CLASS_NAME_SHOW$6)
			) {
				return;
			}
			...
			if (
				...
				[]
					...
					.forEach((elem) =>
						EventHandler.on(elem, 'mouseover', null, noop())
					);
			}
		...
		_getConfig(config) {
			...
			if (
				...
				throw new TypeError(
					`${NAME$7.toUpperCase()}: Option "reference" provided type "object" without a required "getBoundingClientRect" method.`
				);
			}
			...
			for (let i = 0, len = toggles.length; i < len; i++) {
				...
				if ('ontouchstart' in document.documentElement) {
					[]
						.concat(...document.body.children)
						.forEach((elem) =>
							EventHandler.off(elem, 'mouseover', null, noop())
						);
				}
		...
		static dataApiKeydownHandler(event) {
			...
			if (event.key === ESCAPE_KEY$2) {
				const button = this.matches(SELECTOR_DATA_TOGGLE$3)
					? this
			...
			if (
				!isActive &&
				(event.key === ARROW_UP_KEY || event.key === ARROW_DOWN_KEY)
			) {
				const button = this.matches(SELECTOR_DATA_TOGGLE$3)
					? this
					: SelectorEngine.prev(this, SELECTOR_DATA_TOGGLE$3)[0];
	...
	const SELECTOR_FIXED_CONTENT$1 =
		...
		_setElementAttributes(selector, styleProp, callback) {
			SelectorEngine.find(selector).forEach((element) => {
				if (
					...
					window.innerWidth >
						element.clientWidth + this._scrollbarWidth
				) {
					return;
				}

				const actualValue = element.style[styleProp];
				const calculatedValue = window.getComputedStyle(element)[
					styleProp
				];
	...
	class Offcanvas extends BaseComponent {
		...
		_addEventListeners() {
			...
			EventHandler.on(document, EVENT_CLICK_DATA_API$1, (event) => {
				const target = SelectorEngine.findOne(
	...
	EventHandler.on(window, EVENT_LOAD_DATA_API$1, () => {
		SelectorEngine.find(OPEN_SELECTOR).forEach((el) =>
			(Data.get(el, DATA_KEY$5) || new Offcanvas(el)).show()
		);
	});
	/**
	 * ------------------------------------------------------------------------
	...
	const BSCLS_PREFIX_REGEX$1 = new RegExp(
		`(^|\\s)${CLASS_PREFIX$1}\\S+`,
		'g'
	);
